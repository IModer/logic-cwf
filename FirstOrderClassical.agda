...
record Model (i j k l : Level) : Set (lsuc (i ⊔ j ⊔ k ⊔ l)) where
  ...
  field
    ...
    dni : (∀{Γ}{X : For Γ} → Pf Γ (((X ⊃ ⊥) ⊃ ⊥) ⊃ X)) -- ite𝟚 (Pf  ⊤) (∀{Γ}{X : For Γ} → Pf Γ (((X ⊃ ⊥) ⊃ ⊥) ⊃ X)) b
...
module I where
  ...
  [∘]ts : ∀{Γt n}{ts : Tm Γt ^ n}{Δt}{γ : Subt Δt Γt}{Θt}{δ : Subt Θt Δt} → ts [ γ ∘t δ ]ts ≡ ts [ γ ]ts [ δ ]ts

  [∘]t {Γt} {var x} {Δt} {γ} {Θt} {δ} = [∘]v {x = x}
  [∘]t {Γt} {fun n a ts} {Δt} {γ} {Θt} {δ} = cong (fun n a) [∘]ts
  [∘]ts {Γt} {zero} {ts} {Δt} {γ} {Θt} {δ} = refl
  [∘]ts {Γt} {suc n} {ts = t ,Σ ts} {Δt} {γ} {Θt} {δ} = mk,= ([∘]t {t = t}) [∘]ts
  ...
  [id]ts : ∀{Γt n}{ts : Tm Γt ^ n} → ts [ idt ]ts ≡ ts

  [id]t {t = var x} = [id]v
  [id]t {t = fun n a ts} = cong (fun n a) ([id]ts {ts = ts})
  [id]ts {n = zero}                = refl
  [id]ts {n = suc n}{ts = t ,Σ ts} = mk,= [id]t [id]ts
  ...
  data For (Γt : ConTm) : Set where
    ...
    rel  : (n : ℕ) → relar n → Tm Γt ^ n → For Γt
  ...
  data Pf where
    ...
    ∀elim : ∀{Γt}{K Γp} → Pf Γp (∀' K) → (t : Tm Γt) → Pf Γp (K [ idt ,t t ]F)
    ...
    dni : ∀{Γt}{Γp : ConPf Γt}{X : For Γt} → Pf Γp (((X ⊃ ⊥) ⊃ ⊥) ⊃ X)
    qp : ∀{Γt}{Γp : ConPf Γt}{K : For Γt} → Pf (Γp ▸p K) K

  app : ∀{Γ K L}{Γp : ConPf Γ} → Pf Γp (K ⊃ L) → Pf (Γp ▸p K) L
  app m = (m [ pp ]p) $ qp
...
-- Only do enought so that I can be given
-- give I
...
IM = record
  ...
  ; ⊃elim = λ {Γ}{K}{L} pf → substp (Pf (proj₂ Γ ▸p K)) (sym [id]F) (app {proj₁ Γ} {K}{L} {proj₂ Γ} pf)
  ; _∧_    = _∧_
  ; ∧[]    = refl
  ; ∧intro = ∧intro
  ; ∧elim₁ = ∧elim₁
  ...
  ; rel = rel
  ; rel[] = λ {Γ} {_} {_} {_} {Δ} {γ} → relLemma {Γ} {Δ = Δ} {γ = γ}
  ; ∀' = ∀'
  ; ∀[] = refl
  ; ∀intro = ∀intro
  ; ∀elim = un∀
  ...
  ; Eq[] = refl
  ; Eqrefl = ref
  ; subst' = λ K Pfeq Pft → subst' K Pfeq Pft
  ; dni = dni
  }
  where
    open I  