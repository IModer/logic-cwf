...
module FirstOrderLogic.Classical.Model
    (funar : ℕ → Set)
    (relar : ℕ → Set)
    where
...
record Model (i j k l m : Level) : Set (lsuc (i ⊔ j ⊔ k ⊔ l ⊔ m)) where
    ...
    field
        ...
        _[_]F : ∀{Γ Δ} → For Γ → Sub Δ Γ → For Δ   -- Sub Δ Γ → (For Γ → For Δ)
        ...
        -- In Pf we have the additional operation _▸p_ (context extention) 
        -- Pf : For → Prop
    ...
    -- Telescopes of terms
    -- They are isomorphic to Vectors of Tm-s
    -- Its more principled to build these into the theory rather then relaying on our metatheorys Vectors
